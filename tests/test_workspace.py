...
def test_workspace_updates(tmp_path: Path):
    ...
    # Update manifesto
    manager.update_manifesto("Agent 1", turn_num=1, step_label="Thesis", contribution="Proposition: Truth is invariant.")
    updated_manifesto = manager.read_manifesto()
    assert "Proposition: Truth is invariant." in updated_manifesto

    # Update memory
    manager.append_memory("agent_1", "Agent 1", turn_num=1, step_label="Thesis", evolution="Shifted paradigm.")
    ...
    snap_path = manager.save_turn_snapshot(1, 1, "Thesis", "agent_1", res)
